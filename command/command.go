...
func NewCommand(server Server, command string) (cmd *Command) {
	cmd = &Command{
...
func (cmd *Command) Execute(output chan Message) {
	...
	go bindOutput(cmd.Server.ServerName, output, &cmd.Stdout, "", 0)
	go bindOutput(cmd.Server.ServerName, output, &cmd.Stderr, "Error:", console.TextRed)
